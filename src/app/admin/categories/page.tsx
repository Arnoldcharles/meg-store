...
export default function AdminCategoriesPage() {
  ...
  const add = async (e: any) => {
    ...
    const token = await user.getIdToken();
    const res = await fetch("/api/admin/categories", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ name }) });
  ...
  return (
    <div>
      ...
      <form onSubmit={add} className="mb-4">
        <input className="input mr-2" placeholder="New category" value={name} onChange={(e) => setName(e.target.value)} />
      ...
      <ul className="space-y-2">
        {items.map((c) => (
          <li key={c.id} className="flex items-center justify-between p-3 bg-white rounded shadow">
            <span>{c.name}</span>
            <div>
